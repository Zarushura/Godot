...
func shoot():
	...
	flame.get_node("projectile").scale = Vector2(globals.fire_size,globals.fire_size)
	flame.get_node("CollisionShape2D").scale = Vector2(globals.fire_size,globals.fire_size)
...
#			print("max HP ",globals.player_maxhp)
		body.queue_free()
	if body.is_in_group("Xiaphairon"):
		globals.total_score += 20
		globals.player_hp = globals.player_maxhp-0.001
#		globals.fire_rate = globals.fire_rate_cap
#		globals.fire_size = globals.fire_size_cap
#		globals.swing_speed = globals.swing_speed_cap
#		globals.player_speed = globals.player_speed_cap
		body.queue_free()
	if body.is_in_group("Zarushura"):
		globals.total_score += 20
		get_tree().call_group("Devourer", "kill")
		body.queue_free()
