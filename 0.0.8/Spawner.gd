...
var enemyGhost = preload("res://EnemyDevourerGhost.tscn")
var obstacleSeaweed = preload("res://ObstacleSeaweed.tscn")
...
func rarespawn():
	...
	if not globals.please_wait:
		match globals.current_level:
			1:
				var pureType = randi() % 100 + 1
				if (1 <= pureType && pureType <= 50):
					enemy = obstacleRock.instance()
				if (51 <= pureType && pureType <= 100):
					enemy = obstacleRock.instance()
			2:
				var pureType = randi() % 100 + 1
				if (1 <= pureType && pureType <= 50):
				...
				if (51 <= pureType && pureType <= 100):
					enemy = obstacleSeaweed.instance()
