...
var core_xiaphairon = preload("res://CoreXiaphairon.tscn")
var core_zarushura = preload("res://CoreZarushura.tscn")
...
func get_phat_loot(death):
	var powerup
	var roll = (randi() % 100) + 1
	if (1 <= roll && roll <= 49) :
		powerup = core_xiaphairon.instance()
	if (50 <= roll && roll <= 100) :
		powerup = core_zarushura.instance()
	get_tree().get_root().add_child(powerup)
	powerup.global_position = death.global_position
...
func gatechange(world):
	please_wait = true
	checkpoint_score = total_score
